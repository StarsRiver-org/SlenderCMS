...
class User extends Controller {
    ...
    public static function getSession($uuid,$session_id) {
        $session = Db::query("select `session` from slender_group where uid= '" . $uuid . "'");
    ...
    public static function updateSession($uuid,$session_id,$data) {
        ...
        $res = Db::execute("update slender_group set `session` = '".$session."' where `uid` = $uuid");
        return $res;
    }
    ...
    public static function ufetch() {
        if (isset($_COOKIE['uuid'])) {
            $user = Db::query("select * from slender_group where uid= '" . $_COOKIE['uuid'] . "'")[0];
            if(!empty($user)){
    ...
    public static function get_info($uid) {
        ...
        $res = Db::query("select `uid`,`unreg`,`party`,`username`,`promise`,`regtime`,`phone`,`email`,`name` from slender_group where uid = '" . $uid . "'");
        if (!empty($res)) {
            return Qhelp::json_en(['Stat' => 'OK', 'Data' => Qhelp::json_en($res[0])]);
        } else {
    ...
    public static function userup($username, $promise = 100, $password, $name, $email, $phone, $party = 0) {
        if ($promise > 999 || !Qhelp::chk_pint($promise) || (!empty($phone) && (!Qhelp::chk_pint($phone) || strlen($phone) > 15 || strlen($phone) < 6 || $phone < 1000000)) || (!empty($party) && (!Qhelp::chk_pint($party) || $party > 20)) || (!empty($_POST['upuser']) && !Qhelp::chk_pint($_POST['upuser']))) {
        ...
        $pml = Db::query("select `data` from slender_config where `type` = 'Pm' AND `name` = '" . $pml_token[(int)($promise / 100)] . "'")[0]['data'];


        @$self = self::ufetch();
        @$selfpromise = $self['pm'] ? $self['pm'] : 0;
        @$haveuser = Db::query("select username from slender_group where username= '" . $username . "'")[0];/* 返回是否存在用户名- 用户添加 */
        @$udres = Db::query("select * from slender_group where uid= '" . $_POST['upuser'] . "'")[0];/* 返回被操作用户的信息- 用户更新 */
        ...
        if (empty($username)) {
            ...
            if (!empty($name) || !empty($email) || !empty($phone)) {
                if (!empty($_POST['upuser']) && empty($password)) {
                    if ($renew_pm) {
                        Db::execute("update slender_group set `pml` = '$pml' where uid = '" . $_POST['upuser'] . "'");
                    }
                    Db::execute("update slender_group set `username` = '" . $username . "', `promise`= '" . $promise . "', `phone`='" . $phone . "', `email`='" . $email . "', `name`='" . $name . "', `party`='" . $party . "' where uid = '" . $_POST['upuser'] . "'");
                    ...
                    if (!empty($_POST['adduser'])) {
                        Db::execute("INSERT INTO `slender_group`(`username`, `promise`,`pml`, `party`, `salt`, `key`, `phone`, `email`, `name`, `regtime`) VALUE ('" . $username . "','" . $promise . "','" . $pml . "','" . $party . "','" . $salt . "','" . $key . "','" . $phone . "','" . $email . "','" . $name . "','" . date("YmdHi", time()) . "')");
                        Log::visit("consoleboard", "manageuser", "add_" . $username);
                        return Qhelp::json_en(['Stat' => 'OK', 'Message' => '用户添加成功']);
                    } elseif (!empty($_POST['register'])) {
                        Db::execute("INSERT INTO `slender_group`(`username`, `promise`,`pml`, `party`, `salt`, `key`, `phone`, `email`, `name`, `regtime`) VALUE ('" . $username . "','100','" . $pml . "','" . $party . "','" . $salt . "','" . $key . "','" . $phone . "','" . $email . "','" . $name . "','" . date("YmdHi", time()) . "')");
                        ...
                        Db::execute("update slender_group set `username` = '" . $username . "', `promise`= '" . $promise . "', `party`= '" . $party . "', `salt`='" . $salt . "', `key`='" . $key . "', `phone`='" . $phone . "', `email`='" . $email . "', `name`='" . $name . "' where uid = '" . $_POST['upuser'] . "'");
                        Log::visit("consoleboard", "manageuser", "renew_" . $username);
    ...
    public static function userdel($uid) {
        ...
        $res = Db::query("select promise from slender_group where uid = '" . $uid . "'");
        if (!empty($res)) {
            if ($res[0]['promise'] != 999) {
                if ($res[0]['promise'] < User::ufetch()['pm']) {
                    Db::execute("delete from slender_group WHERE uid = '" . $uid . "'");
    ...
    public static function userreg_switcher($uid) {
        ...
        $res = Db::query("select * from slender_group where uid = '" . $uid . "'");
        if ($res[0]['promise'] != 999) {
            if ($res[0]['promise'] < User::ufetch()['pm']) {
                if ($res[0]['unreg'] == 1) {
                    Db::execute("update slender_group set `unreg` = NULL where `uid` = $uid");
                    Log::visit("consoleboard", "manageuser", "un_reg_" . $uid);
                    return Qhelp::json_en(['Stat' => 'OK', 'Message' => '用户解禁成功']);
                } else {
                    Db::execute("update slender_group set `unreg` = 1 where `uid` = $uid");
    ...
    /* 定义细分权限 */
    public static $pml_setting = [/* 你可以在 pml_setting 里面添加新的条目来为用户添加新的权限值。注意按照规则来写。
    ...
    public static function has_pm($pname, $data = 0) {
        ...
        if (in_array($pname, ['is_admin'])) {
            switch ($pname) {
                case 'is_admin':/*是否管理员*/ return $upm >= 700 ? 1 : 0;
                    break;
                default:
                    return 0;
            }
        ...
        switch ($pname) {
            /* 普通用户 */ case 'site_visite': /*浏览网站*/
            case 'thread_visite':/*浏览文章*/
            case 'thread_subscrib':/*评论*/
            case 'user_subscrip':/*评论用户*/ return ($res['value'] || $upm >= $pm_min) ? 1 : 0;
                break;

            /*管理员*/ case 'sms_use':/*使用短信权力*/
            case 'enroll_use':/*参与报名管理权力*/
            case 'config_mag':/*配置管理*/
            case 'user_mag':/*用户管理*/
            case 'nav_mag':/*导航管理*/
            case 'thread_mag':/*文章管理*/
            case 'chunk_mag':/*板块管理*/ return (($res['value'] && $upm >= $pm_min) || $upm >= $pm_std) ? 1 : 0;
                break;
            case 'chunk_ct_mag':/*板块内容管理*/ /* 需要同时拥有 增删改文章 权限 */ if ($upm >= $pm_std) {
                return 1;
            }
                if ($upm >= $pm_min) {
                    $cid = $data;
                    ...
                    foreach ($tes as $v) {
                        $cs = Filter::sample(Db::query("select id from slender_chunk where chunk_below = '“”" . $v . "'"), 'id');
                        $tes = array_merge($tes, $cs);
                        foreach ($cs as $v2) {
                            $cs2 = Filter::sample(Db::query("select id from slender_chunk where chunk_below = '" . $v2 . "'"), 'id');
    ...
    public static function get_pml($uid) {
        ...
        $res = Db::query("select `pml` from slender_group where uid = '" . $uid . "'");
        if (!empty($res)) {
            Log::visit("consoleboard", "manageuser", "getpml_" . $uid);
            return Qhelp::json_en(['Stat' => 'OK', 'Message' => '获取成功', 'Data' => htmlspecialchars_decode($res[0]['pml'], ENT_QUOTES)]);
        } else {
            return Qhelp::json_en(['Stat' => 'error', 'Message' => '用户数据不存在']);
        }
    }
    ...
    public static function set_pml($uid, $pml_json) {
        ...
        // 防止篡改ID导致的权限错乱
        if ($upm = Db::query("select `promise` from slender_group where uid= '" . $uid . "'")) {
            $upm = $upm[0]['promise'];
        } else {
        ...
        /* 获取系统权限条目 */
        $pml_keys = [];
        foreach (self::$pml_setting as $key => $value) {
            $pml_keys[] = $key;
        }
        ...
        foreach ($pml_keys as $value) {
            if (!in_array($value, $pml_ukeys)) {
                $pml[$value] = ['value' => self::$pml_setting[$key]['df'],];
            }
        }
        ...
        Db::execute("update slender_group set `pml` = '" . $pml_json . "' where uid = '" . $uid . "'");
