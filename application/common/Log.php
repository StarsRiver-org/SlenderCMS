    ...
    public static function count_by_timegap($arr, $gap = 0, $loop = 0) {
        ...
        switch ($gap) {
            ...
            case 'day' :
                ...
                $autoloop = 30;
                $current = date("d");
                $short = (date("Y") / 4 == 0 && $mouth[date("m") - 1] == 2) ? $mouth[date("m") - 1] + 1 : $mouth[date("m") - 1];
                break;
            case 'week':
                $gp = 3600 * 24 * 7;
                $autoloop = 15;
                $current = ceil(($today - $toyea) / (3600 * 24 * 7));
                $short = date("Y") / 4 == 0 ? 53 : 52;
                break;
            case 'moon':
                $gp = 3600 * 24 * 30;
                $autoloop = 12;
        ...
        for ($i = 0; $i < $autoloop; $i++) {
            ...
            $total += $count;
            if ($current - $i <= 0) {
                $new_arr[$i] = ["date" => $current - $i + $short, "value" => $count,];
            } else {
                $new_arr[$i] = ["date" => $current - $i, "value" => $count,];
            }
        }
        $new_arr["count"] = $total;
