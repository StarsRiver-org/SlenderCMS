...
class Thread extends Controller {
    ...
    */
    public static function format($res, $type = 0) {
        ...
        switch ($type) {
            case 'sample':
                $return = ['id' => $res['cuid'], 'title' => !empty($res['thread_title']) ? htmlspecialchars_decode($res['thread_title'], ENT_QUOTES) : '', 'time' => date('Y-m-d H:i', $date), 'timestamp' => $date, 'sort' => !empty($res['hk_sort']) ? $res['hk_sort'] : '', 'mode' => !empty($res['hk_mode']) ? $res['hk_mode'] : '', 'editor' => $ut['username'],];
                break;

            case 'more':
                $return = ['id' => $res['cuid'], 'title' => !empty($res['thread_title']) ? htmlspecialchars_decode($res['thread_title'], ENT_QUOTES) : '', 'time' => date('Y-m-d H:i', $date), 'timestamp' => $date, 'coverimg' => @Qhelp::checkpic($res['thread_coverimg']) ? File::fetchimg($res['thread_coverimg']) : STATIC_ROOT . '/img/common/no-img-1.svg', 'keyword' => !empty($res['hk_keywords']) ? htmlspecialchars_decode($res['hk_keywords'], ENT_QUOTES) : '', 'descrip' => !empty($res['hk_descrip']) ? htmlspecialchars_decode($res['hk_descrip'], ENT_QUOTES) : '', 'sort' => !empty($res['hk_sort']) ? $res['hk_sort'] : '', 'hot' => !empty($res['ore_hot']) ? $res['ore_hot'] : 0, 'view' => !empty($res['ore_view']) ? $res['ore_view'] : 0, 'deg' => !empty($res['ore_degree']) ? $res['ore_degree'] : 0, 'mode' => !empty($res['hk_mode']) ? $res['hk_mode'] : '', 'author' => !empty($res['thread_author']) ? $res['thread_author'] : ($ut['name'] ? $ut['name'] : $ut['username']), 'editor' => $ut['username'],];
                break;
            ...
            case 'all':
                ...
                $return = ['id' => $res['cuid'], 'title' => !empty($res['thread_title']) ? htmlspecialchars_decode($res['thread_title'], ENT_QUOTES) : '', 'time' => date('Y-m-d H:i', $date), 'timestamp' => $date, 'coverimg' => @Qhelp::checkpic($res['thread_coverimg']) ? File::fetchimg($res['thread_coverimg']) : STATIC_ROOT . '/img/common/no-img-1.svg', 'content' => !empty($res['thread_context']) ? htmlspecialchars_decode($res['thread_context'], ENT_QUOTES) : '', 'keyword' => !empty($res['hk_keywords']) ? htmlspecialchars_decode($res['hk_keywords'], ENT_QUOTES) : '', 'descrip' => !empty($res['hk_descrip']) ? htmlspecialchars_decode($res['hk_descrip'], ENT_QUOTES) : '', 'sort' => !empty($res['hk_sort']) ? $res['hk_sort'] : '', 'hot' => !empty($res['ore_hot']) ? $res['ore_hot'] : 0, 'view' => !empty($res['ore_view']) ? $res['ore_view'] : 0, 'deg' => !empty($res['ore_degree']) ? $res['ore_degree'] : 0, 'mode' => !empty($res['hk_mode']) ? $res['hk_mode'] : '', 'author' => !empty($res['thread_author']) ? $res['thread_author'] : ($ut['name'] ? $ut['name'] : $ut['username']), 'editor' => $ut['username'], 'like' => $liked,];
