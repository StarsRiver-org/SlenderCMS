...
class Enrollmag_function1 extends Controller {
    ...
    static function refresh($num = 30) {
        ...
        $res = Db::query("select * from slender_usenroll where " . (!empty($cps) ? "campus = $cps AND" : "") . " (hascalled is null OR (isfaced = '-1' AND hascalled = 1)) AND `aim` = $aim limit " . $num);
        ...
        if (!empty($id) && Qhelp::chk_pint($id) && !empty($phone) && Qhelp::chk_pint($phone) && strlen($phone) == 11) {
            ...
            if (!empty($res) && $res[0]["phone"] == $phone) {
                ...
                if (json_decode($smsres, true)['Message'] == 'OK') {
                    ...
                    Db::execute("update slender_usenroll set hascalled = $hascalled, isenrolled = 0, isfaced = 0, f2f = '" . User::ufetch()['uid'] . "', ftime = '" . htmlspecialchars($ftime, ENT_QUOTES) . "' where id = $id");
                }
