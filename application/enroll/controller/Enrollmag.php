...
class Enrollmag extends Controller {
    ...
    public static function chk_pty($faim = null) {
        ...
        if (empty($party)) {
            return Qhelp::json_en(['Stat' => 'No', 'Message' => '你还没有选择自己的部门，无数据返回', "Data" => [],]);
        }
        if (!empty($faim) && $faim != $party) {
            return Qhelp::json_en(['Stat' => 'No', 'Message' => '该面试者不属于你的部门', "Data" => [],]);
        }
        return 0;
    }
    ...
    public function ei() {
        if (!empty($_POST['id']) && Qhelp::chk_pint($_POST['id']) && !empty($_POST['phone']) && Qhelp::chk_pint($_POST['phone']) && strlen($_POST['phone']) == 11) {
            ...
            if (!empty($res) && $res[0]['phone'] == $_POST['phone']) {
                return Qhelp::json_en(['Stat' => 'OK', 'Message' => '加载成功', 'Data' => self::dataFormat($res)[0],]);
            }
        ...
        if (Qhelp::chk_pint($m) && $m > 0) {
            ...
            foreach ($tce as $value) {
                if (!empty($value['ftime']) && !in_array(['cp' => $campus[$value['campus']], 'cpid' => $value['campus'], 'ft' => $value['ftime']], $ftimes)) {
                    $ftimes[] = ['cp' => $campus[$value['campus']], 'cpid' => $value['campus'], 'ft' => $value['ftime']];
                }
            }
            if (!empty($_GET['JSON'])) {
