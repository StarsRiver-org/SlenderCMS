...
class Chunkmag_function extends Controller {
    public static function addchunk() {
        ...
        if (!empty($_POST['chunk_name'])) {
            if ($_POST['add_chunk'] == 'lv1') {
                $check = Db::query("select * from slender_chunk WHERE `type` = '" . $type . "' AND chunk_name = '" . Qhelp::receive('chunk_name') . "' AND chunk_lv = '1'");
                if (!empty($check)) {
                    Re::echo('warning', '名称不能重复', 0);
                } else {
                    Db::execute("insert into slender_chunk(`type`, chunk_name, chunk_lv) VALUE ('" . $type . "', '" . Qhelp::receive('chunk_name') . "','1')");
                ...
                $check = Db::query("select * from slender_chunk WHERE `type` = '" . $type . "' AND chunk_below='" . Qhelp::receive('chunk_below') . "' AND chunk_name = '" . Qhelp::receive('chunk_name') . "'");
                if (!empty($check)) {
                    Re::echo('warning', '名称不能重复', 0);
                } else {
                    Db::execute("insert into slender_chunk(`type`, chunk_name, chunk_lv, chunk_below) VALUE ('" . $type . "', '" . Qhelp::receive('chunk_name') . "','2' , '" . Qhelp::receive('chunk_below') . "')");
                    Log::visit("consoleboard", "managechunk", "add_" . Qhelp::receive('chunk_name'));
                    Re::echo('success', '添加成功', 0);
                }
                ...
                if (!empty($check)) {
                    ...
                    Db::execute("insert into slender_chunk(`type`, chunk_name, chunk_lv, chunk_below) VALUE ('" . $type . "', '" . Qhelp::receive('chunk_name') . "','3', '" . Qhelp::receive('chunk_below') . "')");
                    Log::visit("consoleboard", "managechunk", "add_" . Qhelp::receive('chunk_name'));
                    Re::echo('success', '添加成功', 0);
                }
            } else {
    ...
    public static function delchunk() {
        ...
        $pr = Db::query("select * from slender_chunk WHERE id = '" . $_POST['del_chunk'] . "'");
        if (!empty($pr)) {
            if ($pr[0]['chunk_lv'] == 1) {
                $chunk_lv2 = Db::query("select * from slender_chunk WHERE chunk_below = '" . $_POST['del_chunk'] . "'");
                foreach ($chunk_lv2 as $value) {
                    $chunk_lv3 = Db::query("select * from slender_chunk WHERE chunk_below = '" . $value['id'] . "'");
                    foreach ($chunk_lv3 as $v) {
                        Chunkmag_function::del_thread($v['id']);
                        Db::execute("delete from slender_chunk WHERE id = '" . $v['id'] . "'");
                    }
                    Chunkmag_function::del_thread($value['id']);
                    Db::execute("delete from slender_chunk WHERE id = '" . $value['id'] . "'");
                }
                Chunkmag_function::del_thread($_POST['del_chunk']);
                Db::execute("delete from slender_chunk WHERE id = '" . $_POST['del_chunk'] . "'");
                Log::visit("consoleboard", "managechunk", "del_" . $_POST['del_chunk']);
                Re::echo('success', '删除成功', 0);
            } elseif ($pr[0]['chunk_lv'] == 2) {
                $chunk_lv3 = Db::query("select * from slender_chunk WHERE chunk_below = '" . $_POST['del_chunk'] . "'");
    ...
    public static function combchunk() {
        ...
        $cb = Db::query("select chunk_lv from slender_chunk WHERE id = '" . $_POST['chunk_comber'] . "'");
        $bc = Db::query("select chunk_lv from slender_chunk WHERE id = '" . $_POST['chunk_becomb'] . "'");
        if (!empty($cb) && !empty($bc)) {
            if (Qhelp::receive('chunk_becomb') != Qhelp::receive('chunk_comber')) {
                $chunk_lv = $bc[0]['chunk_lv'];
                if ($chunk_lv == 1) {
                    $chunk_lv2 = Db::query("select * from slender_chunk WHERE chunk_below = '" . Qhelp::receive('chunk_becomb') . "'");
                    foreach ($chunk_lv2 as $value) {
                        $chunk_lv3 = Db::query("select * from slender_chunk WHERE chunk_below = '" . $value['id'] . "'");
                        foreach ($chunk_lv3 as $v) {
                    ...
                    Db::execute("delete from slender_chunk WHERE id = '" . Qhelp::receive('chunk_becomb') . "'");
                    ...
                    $chunk_lv3 = Db::query("select * from slender_chunk WHERE chunk_below = '" . Qhelp::receive('chunk_becomb') . "'");
                    foreach ($chunk_lv3 as $value) {
                        Chunkmag_function::comb_thread($value['id'], Qhelp::receive('chunk_comber'));
                        Db::execute("delete from slender_chunk WHERE id = '" . $value['id'] . "'");
                    }
    ...
    public static function renchunk() {
        ...
        if (!empty($_POST['chunk_rename']) || !empty($_POST['new_name'])) {
            $pr = Db::query("select * from slender_chunk WHERE `type` = '" . $type . "' AND id = '" . Qhelp::receive('chunk_rename') . "'");
            if (!empty($pr)) {
                if (Qhelp::receive('new_name') == $pr[0]['chunk_name']) {
                    Re::echo('warning', '名称未修改', 0);
                } else {
                    if ($pr[0]['chunk_lv'] == 1) {
                        $regd = Db::query("select * from slender_chunk WHERE `type` = '" . $type . "' AND chunk_lv =  1 AND chunk_name = '" . Qhelp::receive('new_name') . "'");
                    } else {
                        $regd = Db::query("select * from slender_chunk WHERE `type` = '" . $type . "' AND chunk_below = '" . $pr[0]['chunk_below'] . "' AND chunk_name = '" . Qhelp::receive('new_name') . "'");
                    }
                    if (empty($regd)) {
                        Db::execute("update slender_chunk set `chunk_name` = '" . Qhelp::receive('new_name') . "' where id = '" . Qhelp::receive('chunk_rename') . "'");
    ...
    public static function del_thread($sort) {
        ...
        $img = Db::query("select thread_coverimg from slender_thread WHERE hk_sort = $sort");
        ...
        Db::execute("delete from slender_thread where hk_sort = $sort");
    }
    ...
    public static function comb_thread($becomb, $comber) {
        ...
        Db::execute("update slender_thread set hk_sort = '" . $comber . "' where hk_sort = $becomb");
    }
}