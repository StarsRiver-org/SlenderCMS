...
class Nav_function extends Controller {
    public static function update($data) {
        ...
        foreach ($arr as $i) {
            if ((!empty($i['id']) && !Qhelp::chk_pint($i['id'])) || (!empty($i['active']) && (!Qhelp::chk_pint($i['active']) || $i['active'] > 1)) || (!empty($i['blank']) && (!Qhelp::chk_pint($i['blank']) || $i['blank'] > 1)) || !Qhelp::chk_pint($i['order']) || !Qhelp::chk_pint($i['type']) || $i['type'] < 1 || !Qhelp::chk_pint($i['bel'])) {
