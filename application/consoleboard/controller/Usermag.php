...
class Usermag extends Controller {
    ...
    public function get_usernum() {
        ...
        return Qhelp::json_en(['num_creator' => $num['num_creator'], 'num_admin' => $num['num_admin'], 'num_editor' => $num['num_editor'], 'num_cuser' => $num['num_user'], 'reg_inday' => $reg_num['regd'], 'reg_inmonth' => $reg_num['regm'],]);
    }
        ...
        if (!empty($_POST['uid']) && is_numeric($_POST['uid']) && $_POST['uid'] > 0) { //判断是否发送了UID
            ...
            if (!empty($beopped_user_pm)) { //判断是否在操作用户对象
                if ($current_user_pm > $beopped_user_pm[0]['promise']) { //判断自身权限是否高于被操作用户权限  （用于只能由高于被操作者的权限者操作的信息）
                    ...
                    /*******************************/
                    if (!empty($_POST['getuserpml'])) {
                        return User::get_pml($_POST['uid']);
                    } //获取用户细分权限
                    if (!empty($_POST['setuserpml'])) {
                        return User::set_pml($_POST['uid'], $_POST['pml']);
                    } //更新用户细分权限
                    if (!empty($_POST['getuserinfo'])) {
                        return User::get_info($_POST['uid']);
                    } //获取用户信息
                    if (!empty($_POST['deluser'])) {
                        return User::userdel($_POST['uid']);
                    } //删除用户
                    if (!empty($_POST['setuserregstat'])) {
                        return User::userreg_switcher($_POST['uid']);
                    } //切换禁封状态
                    ...
                    /*******************************/
                    if (!empty($_POST['setuserregstat'])) {
                        return Qhelp::json_en(['Stat' => 'error', 'Message' => '禁止禁封/解禁自己']);
                    } //切换禁封状态
                    if (!empty($_POST['getuserinfo'])) {
                        return User::get_info($_POST['uid']);
                    } //获取用户信息
            ...
            /*******************************/
            if (!empty($_POST['saveuserinfo'])) {
                return User::userup($_POST['username'], $_POST['usergroup'], $_POST['password'], $_POST['realname'], $_POST['email'], $_POST['phone'], $_POST['party']);
            } //设置用户信息
