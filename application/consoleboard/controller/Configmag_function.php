...
class Configmag_function extends Controller {
    static function saveconf($data) {
        ...
        $rd = Db::query("select * from slender_config where `name` ='" . $name . "'");
        ...
        Db::execute("update slender_config set `data` = '" . htmlspecialchars(Qhelp::dss($data['data']), ENT_QUOTES) . "' where `name`='" . $name . "'");
        return Qhelp::json_en(['Stat' => 'OK', 'Message' => '已将 <b>"' . $name . '"</b> 设置为 <b>"' . htmlspecialchars($data['data'], ENT_QUOTES) . '</b>"']);
    }
    ...
    static function resetconf($data) {
        ...
        Db::execute("update slender_config set `data` = '" . Qhelp::dss($rd[0]['df']) . "' where `name` ='" . $name . "'");
        return Qhelp::json_en(['Stat' => 'OK', 'Message' => $name . ' 》 值已恢复默认', 'Data' => htmlspecialchars_decode($rd[0]['df']),]);
    }
